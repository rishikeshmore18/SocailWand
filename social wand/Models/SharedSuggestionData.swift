...
struct SharedSuggestionData {
    ...
    /// Save suggestion to App Group
    static func save(suggestion: String, sourceApp: String? = nil, tones: [String]? = nil, length: String? = nil) {
        guard let defaults = UserDefaults(suiteName: suiteName) else { return }
        
        defaults.set(suggestion, forKey: suggestionKey)
        defaults.set(true, forKey: hasNewKey)
        defaults.set(Date(), forKey: timestampKey)
        
        if let sourceApp = sourceApp {
            defaults.set(sourceApp, forKey: sourceAppKey)
        }
    }
